...
// A hand-rolled combobox over a Popover, kept because the app's call sites are
// written against this API. shadcn now ships a Combobox on the same Base UI
// primitives, which would replace this file wholesale.
type ComboboxFilter = (
  itemValue: string,
  query: string,
  keywords: string[]
...
function ComboboxInput({
  ...
  const { open, setOpen, setActiveId, listRef, select, disabled, anchorRef } =
    combobox
...
function ComboboxContent({
  ...
  Pick<PopoverPrimitive.Positioner.Props, "align" | "sideOffset"> & {
    // Inside a modal Dialog, pass a host element within the dialog: the
    // dialog's scroll lock only lets wheel events through for targets it
    // contains, so a popup portalled to the body cannot scroll its own list.
