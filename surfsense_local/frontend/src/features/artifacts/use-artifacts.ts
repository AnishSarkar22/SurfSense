...
export function useArtifacts(workspaceId: number) {
  const [formats, setFormats] = useState<ArtifactFormat[]>([])
  ...
  const create = async (job: ArtifactJobCreate) => {
    setIsCreating(true)
    setError(null)
    try {
