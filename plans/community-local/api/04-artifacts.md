...
Accept an artifact job, expose the resulting artifacts, and stream their files —
...
- **`service.py::create_artifact_job(workspace_id, format, document_ids, prompt,
  ...
  provenance from `tool_call_id`), enqueues `artifact_job.delay(artifact_id)`,
...
- **`router.py`**:
  ...
  | Method | Path | Does |
  |---|---|---|
  | `GET` | `/workspaces/{id}/artifacts/formats` | buildable formats + availability from selected model roles; the picker renders from this |
  | `POST` | `/workspaces/{id}/artifacts/jobs` | `{format, document_ids, prompt?, options?}` → 201 artifact |
...
`image_generation`, or null), `available`, nullable `unavailable_reason`, and
`description` (hover copy for the picker). The legacy `requires_key` field is
removed. OpenAPI tag is `artifacts`. There is no `/studio` alias.

## Acceptance

- `POST .../artifacts/jobs` → 201 with an id; polling `GET /artifacts/{id}` flips to
...
Enqueues `artifact_job(artifact_id)` — [`../worker/04-artifacts.md`](../worker/04-artifacts.md).
